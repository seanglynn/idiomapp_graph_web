        ...
        try:
            ...
            try:
                ...
                if model_name in available_models:
                    ...
                    logger.warning(
                        f"Model {model_name} pull initiated but model not immediately available"
                    )
                    return False
            except Exception as e:
                    ...
                    match data:
                        case {"models": models_list} if isinstance(models_list, list):
                            ...
                            for model in models_list:
                                match model:
                                    case {"name": name} if name and isinstance(
                                        name, str
                                    ):
                                        models.append(name)
                                    case _:
                            ...
                            if models:
                                logger.info(
                                    f"Found {len(models)} models using {endpoint}"
                                )
                                return models

                        case {"models": None}:
                            logger.warning(
                                f"API returned None for models list using {endpoint}"
                            )
                        ...
                        case {"tags": None}:
                            logger.warning(
                                f"API returned None for tags list using {endpoint}"
                            )

                        case _:
                            logger.warning(
                                f"Unknown API response format from {endpoint}: {data}"
                            )
...
class OllamaClient:
    ...
    def __init__(self, model_name=None):
        ...
        self.base_url = get_valid_ollama_host()
        ...
        logger.info(
            f"Initialized OllamaClient with model: {self.model_name} at {self.base_url}"
        )
        ...
        try:
            ...
            try:
                ...
                if model_info:
                    self.model_status = "available"
                    logger.info(
                        f"Model {self.model_name} is available (verified with show)"
                    )
            ...
            try:
                ...
                if hasattr(models_response, "models"):
                    for model in models_response.models:
                        if hasattr(
                            model, "model"
                        ):  # New structure uses 'model' attribute
                            model_names.append(model.model)
                        elif hasattr(
                            model, "name"
                        ):  # Older structure used 'name' attribute
                ...
                if not model_names and models_response:
                    ...
                    try:
                        ...
                        if (
                            hasattr(models_response, "models")
                            ...
                            logger.info(
                                f"Extracted model names using string conversion: {model_names}"
                            )
                    except Exception as string_error:
                        logger.error(
                            f"Failed to convert models to strings: {string_error}"
                        )
            ...
            if self.model_name in model_names:
                ...
                logger.warning(
                    f"Model {self.model_name} not found. Attempting to pull it now..."
                )
                try:
                    ...
                    logger.error(
                        f"Failed to automatically pull model: {str(pull_error)}"
                    )
                    logger.warning(
                        f"Model {self.model_name} not available. You can pull it manually with: 'docker exec -it idiomapp-ollama ollama pull {self.model_name}'"
                    )
            ...
            logger.warning(
                f"Could not determine if model {self.model_name} is available."
            )
    ...
    async def generate_text(self, prompt, system_prompt=None):
        ...
        """
        logger.info(
            f"Sending request to model {self.model_name} (prompt length: {len(prompt)})"
        )
...
def get_available_models():
    ...
    try:
        ...
        try:
            ...
            if model_names:
                logger.info(
                    f"Found {len(model_names)} models: {', '.join(model_names)}"
                )
                return model_names
            ...
            if models_response:
                logger.warning(
                    f"Could not extract model names from response: {models_response}"
                )
