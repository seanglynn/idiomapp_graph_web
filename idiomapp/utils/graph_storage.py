...
class GraphStorage(ABC):
    ...
    def get_graph_history(
        self, user_session: str = None, limit: int = 20
    ) -> List[Dict]:
        """
...
class StreamlitGraphStorage(GraphStorage):
    ...
    def store_graph(
        ...
        logger.info(
            f"Stored graph {graph_id} with {len(nodes)} nodes and {len(edges)} edges"
        )
        return graph_id
    ...
    def get_graph_history(
        ...
        if user_session:
            graphs_list = [
                g for g in graphs_list if g.get("user_session") == user_session
            ]
    ...
    def search_graphs_by_text(self, search_text: str, limit: int = 10) -> List[Dict]:
        ...
        for graph in graphs_metadata.values():
            ...
            if (
                search_text_lower in source_text
                or search_text_lower in translation_text
            ):
...
class InMemoryGraphStorage(GraphStorage):
    ...
    def store_graph(
        ...
        logger.info(
            f"Stored graph {graph_id} in memory with {len(nodes)} nodes and {len(edges)} edges"
        )
        return graph_id
    ...
    def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        total_nodes = sum(
            len(self._nodes_data.get(gid, [])) for gid in self._graphs_metadata
        )
        total_edges = sum(
            len(self._edges_data.get(gid, [])) for gid in self._graphs_metadata
        )
