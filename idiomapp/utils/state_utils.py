    ...
    def get_app_state(self) -> AppStateData:
        ...
        if self.state_key not in st.session_state:
            ...
            st.session_state[self.state_key] = AppStateData(
                ...
                last_config_hash=self._generate_config_hash(
                    settings.llm_provider.value, settings.current_model
                ),
            )
...
def get_cached_value(
    ...
    **kwargs,
    ...
    if (
        st.session_state[cache_key] is None
        or current_time - st.session_state[cache_time_key] > cache_duration
    ):
        # Fetch fresh value
...
def reset_application_state() -> None:
    ...
    keys_to_remove = [
        key for key in st.session_state.keys() if key not in essential_keys
    ]
...
def get_state_summary() -> Dict[str, Any]:
    ...
    summary = {
        ...
        "app_state": get_app_state().state.value
    ...
    if "llm_client" in st.session_state and st.session_state["llm_client"] is not None:
        ...
        try:
            ...
            if cached_provider != current_provider or cached_model != current_model:
                logger.info(
                    f"Provider/model changed, clearing cached client: {cached_provider}:{cached_model} -> {current_provider}:{current_model}"
                )
                st.session_state["llm_client"] = None
            else:
                logger.info(
                    f"Using cached LLM client instance for {current_provider}:{current_model}"
                )
